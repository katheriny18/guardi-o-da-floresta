...
function spawnItem(){
  ...
  items.push({ x: Math.random()*360, y: -40, speed: 2 + Math.random()*3 + (level*0.5), type: tipo, isGood: isGood });
}
...
function update(){
  ...
  for(let i=items.length-1; i>=0; i--){
    ...
    if(it.y > player.y && it.y < player.y+40 && it.x > player.x-20 && it.x < player.x+60){
      if(it.isGood){ score += it.type.points; } else { score = Math.max(0, score + it.type.points); lives--; livesEl.textContent = lives; if(lives <= 0){ alert('Game Over! Você fez '+score+' pts'); location.reload(); return; } }
...
function startGame(){
  ...
  gameActive = true;
  update();
  setInterval(()=>{ time--; timeEl.textContent = time; if(time <= 0){ alert('Fim de tempo! Fez '+score+' pts'); location.reload(); } },1000);
}
